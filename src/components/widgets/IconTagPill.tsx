...
import { CATEGORY_ICON_MAP, IconComponent } from "@/utils/categoryHelpers";

interface IconTagPillProps {
  iconKey?: keyof typeof CATEGORY_ICON_MAP;
  label: string;
  icon?: IconComponent;
  iconColor?: string;
  iconSize?: number;
...
const IconTagPill: React.FC<IconTagPillProps> = ({
  ...
  const resolvedIconColor = iconColor || iconConfig?.color || themedColors.secondaryText;
  ...
  return (
    ...
    >
      <IconComponent width={resolvedIconSize} height={resolvedIconSize} color={resolvedIconColor} />
      <ThemedTextSecondary
