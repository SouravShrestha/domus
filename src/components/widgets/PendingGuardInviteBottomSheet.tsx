...
interface PendingGuardInviteBottomSheetProps {
  invite: GuardInvite | null;
  societyName?: string;
...
const PendingGuardInviteBottomSheet: React.FC<
  PendingGuardInviteBottomSheetProps
> = ({ invite, societyName, onDeleteInvite, isLoading = false }) => {
  ...
  const qrValue = useMemo(() => {
    ...
    return `domus://guard-invite/${invite.invite_code}`;
  }, [invite?.invite_code]);
  ...
  const handleShareViaSms = async () => {
    const message = buildGuardInviteMessage(
      invite.name || "there",
      invite.role,
      invite.invite_code,
      societyName,
    );
    const success = await shareViaSms(invite.phone, message);
    if (!success) {
  ...
  const handleShareUniversal = async () => {
    try {
      ...
      await shareUniversal("Domus Guard Invite", message);
    }
  };
  ...
  const handleDeletePress = () => {
    if (isLoading || !onDeleteInvite) return;
    Alert.alert(
      "Delete Invitation",
      `Are you sure you want to delete the invitation for ${invite.name || formatPhoneForDisplay(invite.phone)}?`,
      [
        {
          text: "Cancel",
          style: "cancel",
      ...
      <View className="flex-1" style={{ paddingBottom: insets.bottom + 12 }}>
        <View className="px-6 pt-4">
          <View className="flex-row items-center mb-4">
            <ProfileIcon username={invite.name || "Invited"} size={52} />
            <View className="flex-1 ml-4">
              <ThemedText className="text-lg font-uber-move-medium tracking-wide">
                {invite.name || "Invited Guard"}
              </ThemedText>
...
export default PendingGuardInviteBottomSheet;
