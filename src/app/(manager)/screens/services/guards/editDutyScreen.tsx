...
import { updateGuardDuty } from "@api/services/guard.service";
...
const EditDutyScreen: React.FC = () => {
  ...
  const params = useLocalSearchParams<{
    ...
    currentAllowAnytimeAccess?: string;
  }>();
  ...
  useEffect(() => {
    if (currentGateIds) {
      try {
        const parsedGateIds = JSON.parse(currentGateIds);
        if (Array.isArray(parsedGateIds)) {
          setSelectedGateIds(parsedGateIds);
        }
      } catch {
        // If parsing fails, try splitting by comma
        setSelectedGateIds(currentGateIds.split(",").filter(Boolean));
      }
    }
    if (currentAllowAnytimeAccess === "true") {
      setAllowAnytimeAccess(true);
    }
  }, [currentGateIds, currentAllowAnytimeAccess]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (shiftsResult.data) {
        ...
        // Find matching shift based on current times
        if (currentShiftStart && currentShiftEnd) {
          const matchingShift = shiftsResult.data.find(
            (s) =>
              s.start_time === currentShiftStart &&
              s.end_time === currentShiftEnd,
          );
          if (matchingShift) {
  ...
  }, [societyId, currentShiftStart, currentShiftEnd]);
  ...
  const handleUpdateDuty = async () => {
    if (!assignmentId || selectedGateIds.length === 0 || !selectedShiftId) {
    ...
    Alert.alert(
      "Confirm Update",
      `Update assignment for ${guardName || "this guard"} to ${gateNames} for the ${selectedShift.name} shift?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Update",
          onPress: async () => {
            ...
            try {
              const { error } = await updateGuardDuty(assignmentId, {
              ...
              if (error) {
                Alert.alert("Error", error.message || "Failed to update duty.");
  ...
  return (
    <ThemedView className="flex-1">
      ...
      >
        <View className="-mx-2">
          <ThemedHeaderWithBack title="edit assignment" />
        </View>
