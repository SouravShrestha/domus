...
const buildGuardInviteMessage = (name: string, inviteCode: string): string => {
  const nameText = name ? `Hi ${name}, ` : "";
  return `${nameText}You've been invited to join as a security guard on Domus. Use invite code: ${inviteCode} to accept your assignment.`;
};
...
const InviteSuccessScreen: React.FC = () => {
  ...
  const handleShareViaSms = async () => {
    const message = buildGuardInviteMessage(params.name || "", inviteCode);
  ...
  const handleUniversalShare = async () => {
    try {
      ...
      const message = buildGuardInviteMessage(params.name || "", inviteCode);
      await shareUniversal("Guard Invitation", message);
    }
  };
