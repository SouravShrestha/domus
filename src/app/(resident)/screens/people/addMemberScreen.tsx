...
export type MemberType = "family" | "tenant" | "staff";
...
const AddMemberScreen: React.FC = () => {
  ...
  const params = useLocalSearchParams<{ type: MemberType }>();

  const memberType: MemberType = params.type || "family";
  ...
  const handleContactSelect = useCallback((contact: Contact) => {
    const userPhone = profile?.phone?.replace(/[^0-9]/g, "").slice(-10);
    if (userPhone && contact.phoneNumber === userPhone) {
      showErrorToast("You cannot add yourself");
      return;
    }
  ...
  }, [config.detailsRoute, profile?.phone]);
