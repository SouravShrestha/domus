...
import { ThemedText, ThemedTextSecondary, ThemedView, ThemedHR } from "@themes/themedComponents";
...
const EditFamilyMemberPermissionsScreen: React.FC = () => {
    ...
    const [modifiedPermissions, setModifiedPermissions] = useState<Partial<Record<PermissionKey, boolean>>>({});
    const [hasChanges, setHasChanges] = useState(false);
    const [selectedRole, setSelectedRole] = useState<FamilyRole>(params.memberRole as FamilyRole);
    ...
    const handleRoleChange = (role: FamilyRole) => {
        if (!isOwner || params.memberRole === "owner" || role === selectedRole) return;
        setSelectedRole(role);
        setRoleChanged(role !== params.memberRole);
        const roleDefaults = DEFAULT_PERMISSIONS[role] || {};
        setModifiedPermissions(roleDefaults as Partial<Record<PermissionKey, boolean>>);
        setHasChanges(true);
    };
    ...
    const handleResetToDefaults = () => {
        Alert.alert(
            "Reset to Defaults",
            `Reset permissions to default ${capitalizeFirstLetterOfWords(selectedRole)} settings?`,
            [
    ...
    const renderSection = (title: string, permissions: typeof PERMISSION_INFO) => {
          ...
          {canEdit && (
            ...
            >
              ...
              >
                ...
                {capitalizeFirstLetterOfWords(selectedRole)} defaults
              </ThemedText>
            </TouchableOpacity>
          )}
